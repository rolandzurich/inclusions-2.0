...
# PostgreSQL vollständige Einrichtung auf Server - Single SSH Session
# Server: 10.55.55.155
# User: incluzone
...
# Erstelle ein temporäres Skript, das alle Schritte ausführt
cat > /tmp/postgresql_setup_remote.sh << 'REMOTE_SCRIPT'
#!/bin/bash
set -e
...
# Übertrage das Skript zum Server und führe es aus
echo "Übertrage Setup-Skript zum Server..."
$SSH_SCRIPT "cat > /tmp/postgresql_setup_remote.sh << 'REMOTE_EOF'
$(cat /tmp/postgresql_setup_remote.sh)
REMOTE_EOF
chmod +x /tmp/postgresql_setup_remote.sh
bash /tmp/postgresql_setup_remote.sh"

# Aufräumen
rm -f /tmp/postgresql_setup_remote.sh
