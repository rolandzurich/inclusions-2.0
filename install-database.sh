#!/bin/bash
# Komplettes Datenbank-Setup für Server
# Führe direkt auf dem Server aus: bash install-database.sh
...
for table in "${TABLES[@]}"; do
    EXISTS=$(sudo -u postgres psql -d inclusions_db -tAc "SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = '$table');" 2>/dev/null)
    if [ "$EXISTS" = "t" ]; then
